...
struct ieee80211_hdr {
  ...
  u_int8_t addr3[6];
  u_int16_t seq_ctrl;
  /* followed by 'u_int8_t addr4[6];' if ToDS and FromDS is set in data frame
   */
} STRUCT_PACKED;
...
#define IEEE80211_FC(type, stype) host_to_u_int16_t((type << 2) | (stype << 4))
    ...
    struct {
      ...
      union {
        ...
        struct{
          ...
          u_int8_t new_chan;
          u_int8_t switch_count;
        } STRUCT_PACKED chan_switch;
        struct {
          u_int8_t action;
          u_int8_t sta_addr[ETH_ALEN];
          u_int8_t target_ap_addr[ETH_ALEN];
          u_int8_t variable[0]; /* FT Request */
...
struct ieee80211_ht_capabilities {
  u_int16_t ht_capabilities_info;
  u_int8_t a_mpdu_params;
  u_int8_t supported_mcs_set[16];
  u_int16_t ht_extended_capabilities;
  u_int32_t tx_bf_capability_info;
  u_int8_t asel_capabilities;
} STRUCT_PACKED;


struct ieee80211_ht_operation {
  u_int8_t control_chan;
  u_int8_t ht_param;
  u_int16_t operation_mode;
  u_int16_t stbc_param;
  u_int8_t basic_set[16];
} STRUCT_PACKED;


struct ieee80211_vht_capabilities {
  u_int32_t vht_capabilities_info;
  u_int8_t vht_supported_mcs_set[8];
} STRUCT_PACKED;

struct ieee80211_vht_operation {
  u_int8_t vht_op_info_chwidth;
  u_int8_t vht_op_info_chan_center_freq_seg0_idx;
  u_int8_t vht_op_info_chan_center_freq_seg1_idx;
  u_int16_t vht_basic_mcs_set;
} STRUCT_PACKED;
...
#define ASEL_CAPABILITY_ASEL_CAPABLE ((u_int8_t) BIT(0))
#define ASEL_CAPABILITY_EXPLICIT_CSI_FEEDBACK_BASED_TX_AS_CAP ((u_int8_t) BIT(1))
#define ASEL_CAPABILITY_ANT_INDICES_FEEDBACK_BASED_TX_AS_CAP ((u_int8_t) BIT(2))
#define ASEL_CAPABILITY_EXPLICIT_CSI_FEEDBACK_CAP ((u_int8_t) BIT(3))
#define ASEL_CAPABILITY_ANT_INDICES_FEEDBACK_CAP ((u_int8_t) BIT(4))
#define ASEL_CAPABILITY_RX_AS_CAP ((u_int8_t) BIT(5))
#define ASEL_CAPABILITY_TX_SOUND_PPDUS_CAP ((u_int8_t) BIT(6))

#define HT_INFO_HT_PARAM_SECONDARY_CHNL_OFF_MASK ((u_int8_t) BIT(0) | BIT(1))
#define HT_INFO_HT_PARAM_SECONDARY_CHNL_ABOVE ((u_int8_t) BIT(0))
#define HT_INFO_HT_PARAM_SECONDARY_CHNL_BELOW ((u_int8_t) BIT(0) | BIT(1))
#define HT_INFO_HT_PARAM_REC_TRANS_CHNL_WIDTH ((u_int8_t) BIT(2))
#define HT_INFO_HT_PARAM_RIFS_MODE ((u_int8_t) BIT(3))
#define HT_INFO_HT_PARAM_CTRL_ACCESS_ONLY ((u_int8_t) BIT(4))
#define HT_INFO_HT_PARAM_SRV_INTERVAL_GRANULARITY ((u_int8_t) BIT(5))
...
#define HT_INFO_OPERATION_MODE_NON_GF_DEVS_PRESENT ((u_int8_t) BIT(2))
#define HT_INFO_OPERATION_MODE_TRANSMIT_BURST_LIMIT ((u_int8_t) BIT(3))
#define HT_INFO_OPERATION_MODE_NON_HT_STA_PRESENT ((u_int8_t) BIT(4))
...
struct wmm_information_element {
  ...
  u_int8_t oui[3]; /* 00:50:f2 */
  u_int8_t oui_type; /* 2 */
  u_int8_t oui_subtype; /* 0 */
  u_int8_t version; /* 1 for WMM version 1.0 */
  u_int8_t qos_info; /* AP/STA specific QoS info */
...
struct wmm_ac_parameter {
  u_int8_t aci_aifsn; /* AIFSN, ACM, ACI */
  u_int8_t cw; /* ECWmin, ECWmax (CW = 2^ECW - 1) */
  u_int16_t txop_limit;
} STRUCT_PACKED;
...
struct wmm_tspec_element {
  u_int8_t eid; /* 221 = 0xdd */
  u_int8_t length; /* 6 + 55 = 61 */
  u_int8_t oui[3]; /* 00:50:f2 */
  u_int8_t oui_type; /* 2 */
  u_int8_t oui_subtype; /* 2 */
  u_int8_t version; /* 1 */
  /* WMM TSPEC body (55 octets): */
  u_int8_t ts_info[3];
  u_int16_t nominal_msdu_size;
  u_int16_t maximum_msdu_size;
  u_int32_t minimum_service_interval;
  u_int32_t maximum_service_interval;
  u_int32_t inactivity_interval;
  u_int32_t suspension_interval;
  u_int32_t service_start_time;
  u_int32_t minimum_data_rate;
  u_int32_t mean_data_rate;
  u_int32_t peak_data_rate;
  u_int32_t maximum_burst_size;
  u_int32_t delay_bound;
  u_int32_t minimum_phy_rate;
  u_int16_t surplus_bandwidth_allowance;
...
struct ieee80211_2040_intol_chan_report {
  ...
  u_int8_t length;
  u_int8_t op_class;
  u_int8_t variable[0];	/* Channel List */
} STRUCT_PACKED;

/* IEEE 802.11v - WNM-Sleep Mode element */
struct wnm_sleep_element {
  u_int8_t eid; /* WLAN_EID_WNMSLEEP */
  u_int8_t len;
  u_int8_t action_type; /* WLAN_WNM_SLEEP_ENTER/EXIT */
  u_int8_t status;
  u_int16_t intval;
} STRUCT_PACKED;
